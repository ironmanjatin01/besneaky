...
export default function BackgroundCanvas() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Render floating particles and subtle connections
      particles.forEach((p, i) => {
        ...
        // Particle mouse repulsion / magnetic pull
        const dx = mouseX - p.x
        const dy = mouseY - p.y
        const dist = Math.sqrt(dx * dx + dy * dy)
        if (dist < 120) {
        ...
        ctx.fillStyle = `rgba(61, 57, 53, ${Math.max(0.05, currentOpacity)})`
        ctx.beginPath()
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (pdist < 100) {
            ctx.strokeStyle = `rgba(156, 149, 138, ${0.12 * (1 - pdist / 100)})`
            ctx.lineWidth = 0.6
            ctx.beginPath()
            ctx.moveTo(p.x, p.y)
